...
# Read data
```{r message=FALSE}
df <- read_csv('./US_Accidents_Dec19.csv', col_types = cols(.default = col_character())) %>% type_convert()
```
...
not_useful <- c("ID", "Source", "Timezone", "Airport_Code", "Weather_Timestamp", "Wind_Direction", "Country", "Description")
...
df_time <- df_drop %>%
  ...
  mutate("Year" = str_sub(Date, 1, 4), "Month" = str_sub(Date, 6, 7), "Day" = str_sub(Date, 9, 10), "Wday" = as.character(wday(Date)), "Hour" = str_sub(Time, 1, 2)) %>%
...
df_add %>% filter(is.na(Weather_Condition)) %>% select(Temperature:Weather_Condition) %>% summarise_all(~sum(is.na(.)))
...
```{r}
df_TMC %>% summarise_all(~sum(is.na(.))) %>% pivot_longer(everything(), names_to = "variable", values_to = "NAs") %>% filter(NAs > 0)
...
df_final <- df_TMC %>%
  filter(!is.na(Side)) %>%
  mutate_at(vars(Sunrise_Sunset:Astronomical_Twilight), ~ replace_na(., "NA"))
```
